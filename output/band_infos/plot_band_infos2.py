...
def plot_berry_contour(file_path):
    ...
    try:
        ...
        if data.shape[1] != 12:
            print(f"エラー: ファイル '{file_path}' は12列である必要があります。")
            return
    except Exception as e:
    ...
    for i in range(6,8):
        ...
        # spin=0のデータを青色でプロット
        ax.scatter(kx[spin_num==0], ky[spin_num==0], color='blue', label='Spin 0', s=10)

        # spin=1のデータを赤色でプロット
        ax.scatter(kx[spin_num==1], ky[spin_num==1], color='red', label='Spin 1', s=10)

        discription = "s" + str(i - 5)
    ...
    for i in range(8,12,2):
        ...
        # --- ステップ3: 散乱データをグリッド上に補間 ---
        # 'cubic'は滑らかな補間、'linear'は線形補間
        grid_bcdx = griddata((kx, ky), bcdx, (grid_kx, grid_ky), method='cubic')
        grid_bcdy = griddata((kx, ky), bcdx, (grid_kx, grid_ky), method='cubic')

        # --- ステップ4: 等高線プロットの作成 ---
        fig, ax = plt.subplots(figsize=(8, 7))

        # 塗りつぶし等高線プロット (色)
        contourf = ax.contourf(grid_kx, grid_ky, grid_bcdx, levels=10, cmap='viridis')
    ...
    #         scale=0.05,        # ★スケールをデータの値に合わせて小さく調整
    #         width=0.001
    #     )

    #     discription = "d" + str(int((i - 6)/2))
...
if __name__ == '__main__':
    ...
    plot_berry_contour('dats/a1_2000_tmdsato_lambda0p1_j0p25_mu-0p1.csv')